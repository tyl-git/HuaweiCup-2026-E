"""Train a controlled ordinal-regularized temporal Q2 candidate.
...
contiguous-gap augmentation are unchanged.  The only planned change is using
the train-derived balanced CE plus L1 on complete validation input to choose
the checkpoint, which targets the weak Neutral class without selecting on test.
"""
...
OUT = RESULTS / "q2-ordinal-balanced-v1.0"
...
def main():
    ...
    if args.self_test:
        temporal.self_test()
        print("Balanced-selection self-test OK")
        return
    ...
    if args.preflight:
        print(f"Preflight OK: train={len(train)}, valid={len(valid)}, balanced-selection; no optimizer or output written")
        return
    if OUT.exists() and any(OUT.iterdir()):
        raise FileExistsError(f"Refusing to overwrite prior run: {OUT}")
    OUT.mkdir(parents=True)
    (OUT / "provenance.json").write_text(json.dumps({
        "schema": "q2_ordinal_balanced_v1.0", "train_n": len(train), "valid_n": len(valid),
        "test_evaluated": False, "specialists_evaluated": False,
        "selection_split": "valid_complete", "selection_loss": "ordinary CE + L1; ordinal term optimizer-only",
        "class_counts": counts.tolist(), "class_weights": class_weight.tolist(),
        "architecture": "locked q2 temporal mask-aware gate model with auxiliary ordinal BCE",
    ...
    for seed in SEEDS:
        ...
        for epoch in range(1, config.max_epochs + 1):
            ...
            for rows in torch.randperm(len(train), generator=generator).to(args.device).split(config.batch_size):
                ...
                weighted_ce = torch.nn.functional.cross_entropy(logits, classes,
                        weight=torch.as_tensor(class_weight, device=args.device))
                ordinal_targets = torch.stack((classes >= 1, classes >= 2), dim=1).to(logits.dtype)
                ordinal_logits = torch.stack((logits[:, 1] - logits[:, 0],
                                              logits[:, 2] - logits[:, 1]), dim=1)
                ordinal_bce = torch.nn.functional.binary_cross_entropy_with_logits(
                    ordinal_logits, ordinal_targets)
                loss = (weighted_ce + 0.35 * ordinal_bce
            ...
            weighted_ce = torch.nn.functional.cross_entropy(
                torch.as_tensor(pred["logits"], dtype=torch.float32, device=args.device),
                torch.as_tensor(pred["true_class"], dtype=torch.long, device=args.device),
                weight=torch.as_tensor(class_weight, device=args.device)).item()
            selection = weighted_ce + report["l1"]
            history.append({"epoch": epoch, "valid_balanced_selection_loss": selection,
                            "valid_unweighted_selection_loss": report["selection_loss"], "augmentation": plan})
            print(f"balanced_selection seed={seed} epoch={epoch} valid_balanced_loss={selection:.6f}", flush=True)
            if selection < best - 1e-8:
                best, best_epoch, stale = selection, epoch, 0
                torch.save({"model_state": model.state_dict(), "model": "balanced_selection", "seed": seed,
    ...
    print(f"Ordinal-balanced train/valid complete: {OUT}; test and specialists untouched")
